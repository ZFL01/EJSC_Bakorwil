...
@section('header', 'Edit Akun Admin')
...
<div class="max-w-3xl bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6">
    <div>
        ...
        <p class="text-xs text-gray-500 mt-2">
            Kosongkan field password bila tidak ingin mengubahnya.
        </p>
    </div>

    <form action="{{ route('admin.users.admins.update', $user) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Isi hanya bila ingin mengganti password (min. 8 karakter).</p>
                @error('password')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex justify-end gap-2 pt-4 border-t">
            ...
            <button type="submit"
                    class="px-4 py-2 bg-[#56b8c2] hover:bg-[#3d9aa3] text-white rounded-lg text-sm font-medium transition">Perbarui</button>
        </div>
    </form>
</div>
@endsection
