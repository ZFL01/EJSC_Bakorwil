    ...
    <form action="{{ route('admin.kegiatans.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                ...
                <input type="text" name="judul_kegiatan" value="{{ old('judul_kegiatan') }}" required class="w-full border rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#56b8c2]">
            </div>
            <div>
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="4" class="w-full border rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#56b8c2]">{{ old('deskripsi') }}</textarea>
        </div>
