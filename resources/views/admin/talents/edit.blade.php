    ...
    <form action="{{ route('admin.talents.update', $talent->id_talenta) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="user_name" value="{{ old('user_name', $talent->user->name ?? '') }}" required class="w-full border rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#56b8c2]">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Email User</label>
                <input type="email" name="email" value="{{ old('email', $talent->user->email ?? '') }}" required class="w-full border rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#56b8c2]">
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            @include('partials._bidang-keahlian', [
                'bkFieldKey' => 'bidang-keahlian-talent-edit',
                'bkValue' => $talent->keahlian ?? null,
            ])
            <div>
            ...
            <div>
                ...
                <input type="text" name="sosial_media" value="{{ old('sosial_media', $talent->sosial_media) }}" placeholder="Instagram, Tiktok, Facebook" class="w-full border rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#56b8c2]">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="md:col-span-3">
                ...
                @if($talent->gdrive_src)
                    <p class="text-xs text-gray-500 mb-1">Link saat ini: <a href="{{ $talent->gdrive_src }}" target="_blank" rel="noopener" class="text-[#56b8c2] underline">Buka Google Drive</a></p>
                @endif
                <input type="text" name="url_gdrive" value="{{ old('url_gdrive', $talent->url_gdrive) }}" placeholder="https://drive.google.com/drive/folders/..." class="w-full border rounded-lg p-2 text-sm focus:ring-2 focus:ring-[#56b8c2]">
                <p class="text-xs text-gray-400 mt-1">Link folder/file Google Drive talenta. Kosongkan jika tidak ada.</p>
            </div>
        </div>
