...
<body class="font-sans antialiased bg-gray-50 text-gray-900 overflow-hidden">
    ...
    <!--
        Perubahan utama:
        - main sekarang punya tinggi = (100vh - tinggi navbar) via mt + h-[calc(...)]
          sehingga konten pas dalam satu layar, tidak "full page" yang butuh scroll.
        - overflow-y-auto tetap disiapkan sebagai jaring pengaman kalau layar sangat pendek,
          supaya konten tidak terpotong (tapi pada layar normal tidak akan muncul scrollbar).
    -->
    <main class="mt-14 md:mt-16 h-[calc(100vh-3.5rem)] md:h-[calc(100vh-4rem)] flex items-center justify-center overflow-y-auto px-3 sm:px-4 md:px-6 py-2 bg-gradient-to-br from-[#56b8c2] to-[#3a8a92] mobile-container">
        ...
        <div class="w-full max-w-[700px] max-h-full flex flex-col md:flex-row bg-white rounded-xl overflow-hidden shadow-xl border border-white/20 login-card-mobile">
            ...
            <div class="hidden md:block w-2/5 relative">
                <img alt="EJSC Workspace" class="absolute inset-0 w-full h-full object-cover"
                     src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&h=600&fit=crop&crop=center"
                     style="object-fit: cover;">

                <div class="absolute inset-0" style="background: linear-gradient(to top, rgba(86, 184, 194, 0.9) 0%, rgba(86, 184, 194, 0.3) 50%, transparent 100%);">
                </div>

                <div class="absolute inset-0 flex flex-col justify-end p-5">
                    <h2 class="text-xl font-bold text-white mb-1">Kolaborasi<br/>Tanpa Batas</h2>
                    <p class="text-xs text-white/90 max-w-md">Bergabunglah dengan ekosistem inovasi terbesar di Jawa Timur.</p>
                </div>
            </div>
            ...
            <div class="w-full md:w-3/5 p-4 md:p-5 flex flex-col justify-center overflow-y-auto">
                ...
                <div class="md:hidden flex items-center justify-center mb-3">
                    <img src="{{ Vite::asset('resources/images/logo.png') }}" alt="EJSC Bakorwil" class="h-10 w-auto">
                </div>

                <div class="mb-3 text-center md:text-left">
                    <h1 class="text-lg md:text-xl font-bold text-gray-900 mb-0.5">Selamat Datang</h1>
                    <p class="text-sm text-gray-600">Masuk ke akun EJSC Anda</p>
                </div>

                <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-2.5">
                    @csrf
                    ...
                    <div class="flex flex-col gap-0.5">
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 material-symbols-outlined text-lg">mail</span>
                            <input class="w-full pl-10 pr-3 py-2 bg-gray-50 rounded-lg border border-gray-300 focus:border-[#56b8c2] focus:ring-1 focus:ring-[#56b8c2] outline-none transition-colors duration-200 text-sm text-gray-900 placeholder:text-gray-400"
                                   id="email"
                                   name="email"
                                   value="{{ old('email') }}"
                    ...
                    <div class="flex flex-col gap-0.5">
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 material-symbols-outlined text-lg">lock</span>
                            <input class="w-full pl-10 pr-10 py-2 bg-gray-50 rounded-lg border border-gray-300 focus:border-[#56b8c2] focus:ring-1 focus:ring-[#56b8c2] outline-none transition-colors duration-200 text-sm text-gray-900 placeholder:text-gray-400"
                                   id="password"
                                   name="password"
                                   placeholder="••••••••"
                                   required
                                   type="password">
                            <button aria-label="Toggle password visibility" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" type="button" onclick="togglePassword()">
                    ...
                    <div class="flex items-center justify-between">
                        <label class="flex items-center gap-1.5 cursor-pointer group">
                            <input class="w-4 h-4 rounded border-gray-300 focus:ring-[#56b8c2] transition-colors cursor-pointer"
                                   type="checkbox"
                                   name="remember"
                                   id="remember"
                        ...
                        <a class="text-sm hover:underline transition-all" href="{{ route('forgot-password') }}" style="color: #56b8c2;">Lupa password?</a>
                    </div>
                    ...
                    <div class="relative flex items-center py-1">
                        ...
                        <span class="flex-shrink-0 px-2 text-xs text-gray-500">Atau masuk dengan</span>
                        <div class="flex-grow border-t border-gray-200"></div>
                    </div>

                    <!-- SSO Buttons -->
                    <div class="flex gap-2">
                        <button class="flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors text-gray-700 text-sm" type="button">
